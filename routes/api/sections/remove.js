                ...
                FROM sections LEFT JOIN files ON sections.section_id = files.section_id WHERE sections.section_id = ?`, [sectionId], (results, res) => {
                    ...
                    for (var i = 0; i < results.length; i++) {
                        if (results[i].file_id !== null) {
                            files.push({"id": results[i].file_id, "title": results[i].file_name, "path": results[i].file_link});
                        }
                    }
                    let secRemove = JSON.stringify({"id": results[0].section_id, "name": results[0].section_name, "text": results[0].article_text, "position": results[0].position, "files": files});
                    const queryString = 'DELETE FROM sections WHERE section_id = ?';
                    const queryString2 = 'DELETE FROM files WHERE section_id = ?';
                    utils.mysql_query(res, queryString, [sectionId], (results, res) => {utils.log(userId, utils.actions.REMOVE, utils.entities.SECTION,'{}',secRemove); res.sendStatus(200);});
