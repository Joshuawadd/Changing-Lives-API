            ...
            if (!isNaN(userId)) {
                utils.mysql_query(res,'SELECT * FROM users WHERE is_admin = 1',[], (adminUsers, res) => {
                    ...
                    utils.mysql_query(res,'SELECT * FROM users WHERE user_id = ?',[userId], (results, res) => {
                        let userRemove = JSON.stringify({"id": results[0].user_id, "nickname": results[0].real_name, "name": results[0].username, "password": results[0].password, "passwordSalt": results[0].password_salt, "isAdmin": results[0].is_admin.readUInt8()});
